...
public class SimpleGraphicsAdapter implements Canvas {
    ...
    private final ArrayDeque<Consumer<Graphics2D>> consumerQueue = new ArrayDeque<>();
    ...
    public void drawLine(int x1, int y1, int x2, int y2, EColor color) {
        ...
        // we sore action of draw
        this.consumerQueue.add(graphics2D -> {
    ...
    public void render() {
        this.simpleGraphics.render(graphics2D -> {
            while (!this.consumerQueue.isEmpty()) {
                this.consumerQueue.pop().accept(graphics2D);
            }
        });
    }
}
